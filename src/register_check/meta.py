...
from register_check.asserts_command import WorkflowStep, _suppression_match, _workflow_steps
from register_check.register import Control, MetaControl, Register
from register_check.remote import GitHub, NoCredentials, Unreadable, Unresolvable
from register_check.repo import Repo, git
from register_check.rulesets import by_rule_type, enforced_contexts
from register_check.runner import Verdict, applies
...
#
# The leading alternation is what keeps `pip install register-check` out: an
...
_FULL_RUN = re.compile(
    r"(?:^\s*|[;&|(]\s*)(?:uv\s+run\s+)?register-check"
...
def _reaches(control: Control, run: str, register: Register) -> bool:
    ...
    every in-process assertion was declared as `register-check assert …`, meant
    six controls collapsed to the single token `register-check`, and the two
    ...
    for block in control.verify:
        if block.kind == "command" and block.run and _invocation(block.run.split()[0]).search(run):
            return True
        # `register-check assert <name>` — the debugging entry point, but a
        # legitimate way for CI to reach one assertion deliberately.
        if (
            block.kind == "file"
            and block.assert_name
            and re.search(rf"register-check\s+assert\s+{re.escape(block.assert_name)}\b", run)
        ):
            return True
    return False
...
def gov_001(
    ...
    for control in register.controls:
        ...
        # check, or a swallowed `register-check` would be credited with reaching
        # everything.
        reaching = [
            step
