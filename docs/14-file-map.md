...
| `scripts/` | `scripts/plan_progress.py`, the derived view of the build plan's exit criteria; `scripts/craft_scaffold.py`, which materialises S3's bench scaffolds into gitignored `temp/` — they cannot be tracked, and `docs/craft/review.bench.md` § What the criteria are measured on says why |
