...
- The repository has neither a `Dockerfile` nor a `.devcontainer/`. Both
  ...
  nobody asked for.
- You want every gate, not these two. Use `standard-adopt`, which plans across
...
4. `standard-check run --control BLD-001 --control DEV-001` was run afterwards,
...
```bash
standard-check --repo "$REPO" --register "$REGISTER" explain BLD-001
```
...
Verified: standard-check run --control BLD-001 --control DEV-001 → exit 0
```

**Failed verification:**

```text
gate-build wrote its artefacts, and standard-check does not accept them.
