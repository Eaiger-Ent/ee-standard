...
description: >
  ...
  through standard-check. Triggers: 'deploy gate-quality', '/gate-quality'.
...
```bash
standard-check --repo "$REPO" --register "$REGISTER" explain LNT-001
```
...
Verified: standard-check run --control LNT-001 --control TYP-001
...
```text
gate-quality wrote its artefacts, and standard-check does not accept them.
