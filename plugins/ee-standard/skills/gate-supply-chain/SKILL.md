...
- The repository has no package manager at all. SUP-001 passes on that finding
  ...
  alone.
- You want every gate, not these three. Use `standard-adopt`, which plans across
...
4. `standard-check run --control SUP-001 --control SUP-002 --control SUP-003`
...
```bash
standard-check --repo "$REPO" --register "$REGISTER" explain SUP-001
```
...
Verified: standard-check run --control SUP-001 --control SUP-002 \
...
```text
gate-supply-chain wrote its artefacts, and standard-check does not accept them.
