...
# /register-adopt — the front door

You are running the **register-adopt** skill. It is the only entry point a user
...
4. `register-check` was run afterwards over the whole register, its output shown,
...
```bash
register-check --repo "$REPO" --register "$REGISTER" schema
register-check --repo "$REPO" --register "$REGISTER" run
```
...
costs: the controls left out keep failing, and `register-check` will exit
...
```text
register-adopt deployed <n> controls in <repo>.
...
```text
register-adopt stopped at <gate>: <the failing block, verbatim>.
...
```text
register-adopt showed the plan and wrote nothing.
```
