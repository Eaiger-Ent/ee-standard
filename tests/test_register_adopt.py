"""`register-adopt` end to end: plan → deploy → verify, on a scratch repository.
...
SKILL = PLUGIN / "skills/register-adopt"
...
def _adopter_register(root: Path) -> Path:
    ...
    `register-check` stays, whose authority is the lockfile the adopter commits.
    """
    ...
    # checks the branch must require, and those are job ids — `register-check`
