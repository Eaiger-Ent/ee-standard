...
| `spectral` | <https://docs.stoplight.io/docs/spectral/674b27b261c3c-overview> | Apache-2.0 | `@stoplight/spectral-cli` 6.16.3, published to npm 2026-08-03; repository pushed 2026-09-05 | tool | Linting an OpenAPI or AsyncAPI document against a rule set. The instrument behind the API rows in `assess.rules.md`, and it can say nothing about an API with no description document. **A count stood here until 2026-09-07**; a source register counting another document's rows is a second copy of that document's state, and this one had drifted |
