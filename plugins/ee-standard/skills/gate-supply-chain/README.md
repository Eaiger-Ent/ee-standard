...
So the pre-commit hook runs `standard-check run --control SUP-003`: the same
pinned binary, the same assert, one implementation. It is reached through the
register's `tools.standard-check.invocation` rather than by name, because a bare
...
Running the checker is not the same as auditing with it. `standard-check
...
pre-commit config ran `standard-check schema` and would otherwise have been
