...
4. `standard-check run --control IAC-001` was run afterwards, its output shown,
...
```bash
standard-check --repo "$REPO" --register "$REGISTER" explain IAC-001
```
...
```bash
standard-check --repo "$REPO" --register "$REGISTER" run --control IAC-001
```
...
Verified: standard-check run --control IAC-001 → exit 0
```
...
```text
gate-iac wrote its artefacts, and standard-check does not accept them.
